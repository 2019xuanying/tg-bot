...
async def main_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_ctrl_plugins":
        ...
        keyboard = [
            ...
            [InlineKeyboardButton(f"Flexiroam: {'🟢' if f_status else '🔴'}", callback_data="admin_toggle_flexiroam")],
    ...
    if data == "admin_proxy_import":
        ...
        await query.edit_message_text("📥 **请直接回复代理列表**\n\n每行一个：`ip:port:user:pass` 或 `ip:port`", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 取消", callback_data="admin_ctrl_proxies")]]), parse_mode='Markdown')
        return
...
def main():
    ...
    ivideo.register_handlers(application) # 注意这里是小写，与导入一致

    print("✅ 机器人已启动 (All Plugins Ready)...")
    application.run_polling()

if __name__ == '__main__':
    main()ADMIN_WAIT_PROXY_LIST = 101
